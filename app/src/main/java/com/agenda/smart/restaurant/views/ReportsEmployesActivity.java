...
public class ReportsEmployesActivity extends AppCompatActivity implements View.OnClickListener,NavigationView.OnNavigationItemSelectedListener {
    TextView dateFrom, dateTo, resturantName;
    Button search;
    SharedPreferences dataSaver;
    EmployesReportsAdapter employesReportsAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_reports_employes);
        ...
        recyclerEmployees = findViewById(R.id.employees_recycle);
    ...
    public void onClick(View view) {
        ...
        if (view == search) {
            ...
            Log.e("TAG", "date_to " + date_to);
            Log.e("TAG", "date_from " + date_from);
            Service.Fetcher.getInstance().getReportEmployees(api_token, date_from, date_to).enqueue(new Callback<ReportsEmployees>() {
                @Override
                public void onResponse(Call<ReportsEmployees> call, Response<ReportsEmployees> response) {
                    if (response.isSuccessful()) {
                        progressDialog.dismiss();
                        ReportsEmployees taxes;
                        taxes = response.body();

                        if (taxes.getEmployees().size() == 0) {
                            Toast.makeText(ReportsEmployesActivity.this, "لا يوجد تقارير لهذه الفترة", Toast.LENGTH_SHORT).show();
                        }

                        employesReportsAdapter = new EmployesReportsAdapter(ReportsEmployesActivity.this, taxes);
                        recyclerEmployees.setAdapter(employesReportsAdapter);

                    } else {
                        progressDialog.dismiss();
                        Toast.makeText(ReportsEmployesActivity.this, "يجب ان تكون مسجل الدخول اولا", Toast.LENGTH_SHORT).show();
                    }
                }

                @Override
                public void onFailure(Call<ReportsEmployees> call, Throwable t) {
                    progressDialog.dismiss();
                    Log.e("TAG ", "onFailure" + t.getMessage());
                    Toast.makeText(ReportsEmployesActivity.this, "خطأ", Toast.LENGTH_SHORT).show();
                }
